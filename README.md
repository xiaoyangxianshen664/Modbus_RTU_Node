...
设备地址默认为 `1`，寄存器地址从 `0` 开始。所有 Modbus 多字节数据使用大端序，帧尾使用 Modbus CRC16（低字节在前）。

### 保持寄存器（Holding，4 个）

使用功能码 `0x03` 读取，使用 `0x06` 写单个寄存器或 `0x10` 批量写入。写入成功后，配置会校验并保存到 W25Q256 的 A/B 双槽，掉电重启后自动恢复。

| 地址 | 含义 | 单位/范围 | 默认值 |
| --- | --- | --- | --- |
| `0x0000` | ADC 采集周期 | ms，必须大于 0 | `1000` |
| `0x0001` | 温度下限 | 0.1 ℃，`20.0~80.0 ℃` | `200`（20.0 ℃） |
| `0x0002` | 温度上限 | 0.1 ℃，`20.0~80.0 ℃` | `400`（40.0 ℃） |
| `0x0003` | SD 日志开关 | `0=关闭，1=开启` | `0` |

### 输入寄存器（Input，12 个）

使用功能码 `0x04` 读取，只读，由采集、监控和日志任务更新。

| 地址 | 含义 | 单位/说明 |
| --- | --- | --- |
| `0x0000` | 电位器原始值 | ADC counts |
| `0x0001` | 电位器工程量 | 0.1 %，例如 `835=83.5%` |
| `0x0002` | NTC 原始值 | ADC counts |
| `0x0003` | 温度 | 0.1 ℃，例如 `291=29.1 ℃` |
| `0x0004` | 报警状态 | `0=正常，1=低温，2=高温，3=上下限配置错误` |
| `0x0005~0x0007` | 预留 | 当前未使用 |
| `0x0008` | 最近一次复位原因 | 复位诊断值 |
| `0x0009` | 健康故障次数 | MonitorTask 统计 |
| `0x000A` | SD/日志错误次数 | LogTask 统计 |
| `0x000B` | 当前健康状态 | `1=正常，0=异常` |

### 功能码与报错机制

- `0x03`：读取保持寄存器；`0x04`：读取输入寄存器。
- `0x06`：写单个保持寄存器；`0x10`：批量写保持寄存器，批量写入采用“先全部校验、后统一提交”，不会留下半组配置。
- 非法功能码返回异常功能 `原功能码 | 0x80` 和异常码 `0x01`（非法功能）。
- 寄存器地址越界、数量为 0 或超出范围返回异常码 `0x02`（非法地址）。
- 保持寄存器参数不符合范围返回异常码 `0x03`（非法数据值）。
- CRC 错误、帧间隔错误或帧格式错误不执行写入，也不发送正常响应。
- 从站地址 `0x00` 为广播地址，仅允许写操作并且不回传响应；读取请求不会作为广播执行。
