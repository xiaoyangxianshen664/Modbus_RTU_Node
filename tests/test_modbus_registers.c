...
static int test_read_and_update(void)
{
    ...
    TEST_ASSERT_EQ(UINT16_C(200), values[1]);                            // 报警下限默认 20.0℃
    return 0;
}
 ...
 *
 * 覆盖：
 *   合法值 {300, 400} 写入成功（30.0℃~40.0℃）
 *   非法值 {300, 100}（地址 2 低于允许的 20.0℃）→ 整体拒绝，一个都不写
 *   读越界（起始 3 读 2 个）→ 返回 INVALID_ADDRESS
 * ════════════════════════════════════════════════════════════ */
static int test_write_is_atomic_and_validated(void)
{
    modbus_registers_t registers;
    const uint16_t valid_values[] = { UINT16_C(300), UINT16_C(400) };  // 合法：地址 1=30.0℃，地址 2=40.0℃
    const uint16_t invalid_values[] = { UINT16_C(300), UINT16_C(100) }; // 非法：地址 2 低于 20.0℃
    uint16_t values[2];
    ...
    TEST_ASSERT_EQ(UINT16_C(300), values[0]);
    TEST_ASSERT_EQ(UINT16_C(400), values[1]);
    ...
    TEST_ASSERT_EQ(UINT16_C(400), values[0]);                            // 地址 2 还是原来写的 400
